...
int main(int argc, char *argv[]) {
  ...
  while (!err && (arg = getopt(argc, argv, ":e:ivclnhsf:o")) != -1) {
    switch (arg) {
      case 'f':
        options.f = 1;
        if ((err = read_file_templates(templates, &count_templates, optarg)))
...
int print_matches(regex_t *templates, int count_templates, char *filename) {
  ...
  char line[MAX_LINE_LENGHT], match = 0;
  FILE *file = fopen(filename, "r");

  !file ? (result = 0) : (result = 1);

  while (result && feof(file) == 0 && fgets(line, MAX_LINE_LENGHT, file)) {
    match = 0;
...
int read_file_templates(regex_t *templates, int *count_templates,
  ...
  while (result && feof(f) == 0 && fgets(line, MAX_LINE_LENGHT, f)) {
    line_len = strlen(line);
